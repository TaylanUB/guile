...
static void set_port_curly_infix_p (SCM port, scm_t_read_context *ctx,
                                    int value);
static void set_port_r6rs_hex_escapes_p (SCM port, scm_t_read_context *ctx,
                                         int value);
static void set_port_hungry_eol_escapes_p (SCM port, scm_t_read_context *ctx,
                                           int value);
static void set_port_keyword_style (SCM port, scm_t_read_context *ctx,
          ...
          if (0 == strcmp ("r6rs", name))
            {
              ...
              set_port_r6rs_hex_escapes_p (port, ctx, 1);
              set_port_square_brackets_p (port, ctx, 1);
              set_port_keyword_style (port, ctx, KEYWORD_STYLE_HASH_PREFIX);
              set_port_hungry_eol_escapes_p (port, ctx, 1);
            }
...
/* Set CTX and PORT's r6rs_hex_escapes_p option according to VALUE. */
static void
set_port_r6rs_hex_escapes_p (SCM port, scm_t_read_context *ctx, int value)
{
  value = !!value;
  ctx->r6rs_escapes_p = value;
  set_port_read_option (port, READ_OPTION_R6RS_ESCAPES_P, value);
}

static void
set_port_hungry_eol_escapes_p (SCM port, scm_t_read_context *ctx, int value)
{
  value = !!value;
  ctx->hungry_eol_escapes_p = value;
  set_port_read_option (port, READ_OPTION_HUNGRY_EOL_ESCAPES_P, value);
}

static void
set_port_keyword_style (SCM port, scm_t_read_context *ctx, enum t_keyword_style value)
{
  ctx->keyword_style = value;
